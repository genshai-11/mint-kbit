...
const UI: Record<string, Tri> = {
  ...
  'Inquiry received by the KBIT coordination team.': { vi: 'Đội điều phối KBIT tiếp nhận yêu cầu.', ko: 'KBIT 코디네이션 팀이 문의를 접수합니다.' },
  'Membership, event, or partnership request is routed to the right office.': { vi: 'Yêu cầu về hội viên, sự kiện hoặc hợp tác được chuyển đến văn phòng phù hợp.', ko: '회원, 행사 또는 파트너십 문의가 담당 사무소로 전달됩니다.' },
  'A coordinator follows up with the next action or required documents.': { vi: 'Điều phối viên sẽ liên hệ lại với bước tiếp theo hoặc tài liệu cần chuẩn bị.', ko: '담당자가 다음 단계 또는 필요한 서류를 안내합니다.' },
